<script>
	...
	const buildPayload = (formData) => {
		...
		for (const [fieldName] of Object.entries(formChanges)) {
		...
		if (formChanges.brand) {
			if (
				isEmpty(formData.active_ingredient) &&
				!isEmpty(activeIngredient)
			) {
				payload['active_ingredient_root_id'] = activeIngredient[0].id;
				payload['drug_combination_regimen_root_id'] = null;
			}
			if (
				isEmpty(formData.drug_combination_regimen) &&
				!isEmpty(drugCombination)
				...
				payload['active_ingredient_root_id'] = null;
			}
		}
	...
	const confirmSubmit = async () => {
		...
		loader.show();
		try {
			await updateApprovedStudy(id, payload);
			await fetchdata();
	...
	const fetchdata = async () => {
		try {
			loader.show();
			const response = await fetchApprovedStudy(id);
			...
			if (!isEmpty(initialData.brand)) {
				initialData.brand = {
					value: initialData.brand[0].root_id,
					label: initialData.brand[0].root_name,
				};
			} else {
			...
			if (!isEmpty(initialData.active_ingredient)) {
				...
				activeIngredient = [
					{
						id: initialData.active_ingredient[0].root_id,
						name: initialData.active_ingredient[0].root_name,
					},
				];
			}
			if (!isEmpty(initialData.drug_combination_regimen)) {
				drugCombination = [
					{
						id: initialData.drug_combination_regimen[0].root_id,
						name: initialData.drug_combination_regimen[0].root_name,
					},
				];
			}
			...
			if (!isEmpty(formData.review_designations)) {
				formData.review_designations = [
					...formData.review_designations,
				];
			}
		} catch (error) {
